...
author		:= Chris Monson
version		:= 2.1.22.1
svninfo		:= $$Id: Makefile 43 2008-09-03 20:26:37Z shiblon $$
#
...
#
# CHANGES:
# Anielkis Herrera <aherrerag@uci.cu> (2009-11-24):
# 	* Added support for glossaries package
...
# Outputs all glossaries files to stdout.  Arg 1 is the source stem, arg 2 is the
# list of targets for the discovered dependency.
#
# $(call get-glos,<parsed file>,<target files>)
define get-glos
$(SED) \
-e 's/^No file \(.*\)\.glo\.$$/$2: \1.glo/p' \
-e 'd' \
$1 | $(SORT) | $(UNIQ)
endef
...
# Colorize Makeindex errors
define colorize-makeglossaries-errors
$(SED) \
-e '/^!! /{' \
-e '  N' \
-e '  s/^.*$$/$(C_ERROR)&$(C_RESET)/' \
...
# $(call run-makeglossaries,<tex stem>)
define run-makeglossaries
success=1; \
if ! $(MAKEGLOSSARIES) $1 > /dev/null; then \
	$(call colorize-makeglossaries-errors,$1.ilg); \
	success=0; \
fi; \
[ "$$success" = "1" ] && $(sh_true) || $(sh_false)
endef
...
%.d %.aux %.aux.make: %.tex
	...
	$(call get-glos,$*.log,$(addprefix $*.,d aux aux.make)) >> $*.d; \
